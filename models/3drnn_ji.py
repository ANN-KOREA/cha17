...
data_loader = Loader(
    ...
    preprocessors=[
        ...
        VideoLoadPrep(n_frames=n_frames, rand_middle_frame=True, rgb=n_channels>1,
                      use_bcolz=True, tolerance=0),
        Augment3D(output_shape=(n_frames,)+im_shp, output_scale=(1,1,1), mode="nearest",
                  interp_order=1,
...
def build_objectives(model, deterministic):
    ...
    # Jaccard Index
    targets1h = T.extra_ops.to_one_hot(targets, voc_size)
    targets1hsum = T.sum(targets1h, axis=0)
    non_zero_idxs = T.flatnonzero(targets1hsum)

    intersection = T.sum(targets1h * preds, axis=0) # per class
    union = targets1hsum + T.sum(preds, axis=0) - intersection

    loss = intersection / (union+1e-6)
    loss = np.float32(1.) - loss.sum() / non_zero_idxs.shape[0]

    # toprint["nzi"] = non_zero_idxs.shape[0]
...
def residual(l_in, num_filters, normalization=None, filter_size=3, stride=1, num_layers=2,
    ...
    for _ in range(num_layers):
        if sep:
            l = _conv3d(l, stride=(1,1,stride[2]), filter_size=(1,1,filter_size), num_filters=num_filters, pad='same', nonlinearity=None, W=W, b=None)
            l = _conv3d(l, stride=(1,stride[1],1), filter_size=(1,filter_size,1), num_filters=num_filters, pad='same', nonlinearity=None, W=W, b=None)
