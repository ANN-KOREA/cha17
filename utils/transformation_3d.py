...
def apply_affine_transform(x, matrix, output_shape=None, **kwargs):
    output_shape = np.asarray(output_shape, np.int)
    # output.dot(T) + s = input
    T = matrix[:3, :3]
    s = matrix[:3, 3]
    return scipy.ndimage.interpolation.affine_transform(x, T, offset=s, output_shape=output_shape, **kwargs)
...
def test_transformation_3d():
    ...
    import utils.plt
    utils.plt.show_animate(img)
    utils.plt.show_animate(img_trans)
